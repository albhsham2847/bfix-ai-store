...
export default function ProductCard({ p }: { p: ProductCardData }) {
  return (
    ...
    >
      <div
        className={`grid h-14 w-14 shrink-0 place-items-center rounded-xl bg-gradient-to-br ${p.gradient ?? "from-gold to-gold-2"} text-2xl shadow-lg`}
      >
        {p.icon ?? "✨"}
      </div>
      <div className="min-w-0 flex-1">
        ...
        {p.categoryName && (
          <p className="mt-0.5 text-[11px] text-white/40">{p.categoryName}</p>
        )}
      </div>
