...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 px-4 pb-[calc(env(safe-area-inset-bottom)+0.75rem)]">
      <div className="glass mx-auto flex max-w-md items-center justify-around rounded-2xl px-2 py-2 shadow-2xl shadow-black/60">
        {items.map((it) => {
          ...
          return (
            <Link
              key={it.href}
              href={it.href}
              className={`flex flex-col items-center gap-0.5 rounded-xl px-3 py-1.5 text-[11px] font-bold transition ${
                active ? "gold-btn" : "text-white/60"
              }`}
            >
