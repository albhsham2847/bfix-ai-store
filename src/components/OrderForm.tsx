...
export default function OrderForm({
  ...
  /* ---------- FORM ---------- */
  return (
    <form onSubmit={goSummary} className="glass fade-up space-y-3 rounded-3xl p-5">
      <h2 className="text-base font-black">اطلب الآن</h2>

      {options.length > 0 && (
        <div>
          <div className="mb-1.5 text-xs font-bold text-white/60">اختر المدة / الباقة</div>
      ...
      <div className="flex items-center justify-between rounded-xl bg-black/30 px-4 py-2 ring-1 ring-white/10">
        <span className="text-sm text-white/70">الكمية {unit && !option ? `(${unit})` : ""}</span>
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => setQty((q) => Math.max(1, q - 1))} className="h-8 w-8 rounded-lg bg-white/10 text-lg font-black">−</button>
          <span className="w-6 text-center font-black">{qty}</span>
          <button type="button" onClick={() => setQty((q) => Math.min(100, q + 1))} className="h-8 w-8 rounded-lg bg-white/10 text-lg font-black">+</button>
        </div>
      </div>

      <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="الاسم الكامل *" className={input} />
      <input required type="tel" dir="ltr" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+967 7xx xxx xxx *" className={`${input} text-right`} />
      <input type="email" dir="ltr" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="البريد الإلكتروني (اختياري)" className={`${input} text-right`} />
      {requiredInfo && (
        <div>
          <div className="mb-1 text-xs font-bold text-gold">📝 مطلوب: {requiredInfo}</div>
          <textarea value={customerInput} onChange={(e) => setCustomerInput(e.target.value)} rows={2} placeholder={requiredInfo} className={input} />
        </div>
      )}
      <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="ملاحظات إضافية (اختياري)" rows={2} className={input} />

      {error && <p className="text-sm font-bold text-rose-400">{error}</p>}
      <button className="gold-btn w-full rounded-xl px-4 py-3.5 text-base font-black">
