...
export default function CustomerAuthModal() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: name.trim(), phone: regPhone.trim(), email: regEmail.trim(), password: regPassword }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "خطأ");
      login(data.customer);
  ...
  async function handleForgot(e: React.FormEvent) {
    ...
    if (newPassword.length < 6) return setError("كلمة المرور 6 أحرف على الأقل");
    setLoading(true);
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "خطأ");
      setSuccess("تم إعادة التعيين — يمكنك تسجيل الدخول الآن");
        ...
        {customer ? (
          ...
          <form onSubmit={handleRegister}>
            ...
            <div className="space-y-3">
              ...
              <input required type="password" value={regPassword} onChange={(e) => setRegPassword(e.target.value)} placeholder="كلمة المرور (6 أحرف) *" className={ic} />
              ...
              <button disabled={loading} className="gold-btn oneui-btn w-full disabled:opacity-60">{loading ? "..." : "إنشاء الحساب"}</button>
              <button type="button" onClick={() => switchView("login")} className="oneui-btn w-full"
                style={{ background: "var(--surface-2)", color: "var(--text-secondary)" }}>لدي حساب — تسجيل الدخول</button>
            </div>
          </form>
        ) : view === "forgot" ? (
          <form onSubmit={handleForgot}>
            ...
            <div className="space-y-3">
              <input required type="tel" dir="ltr" value={forgotPhone} onChange={(e) => setForgotPhone(e.target.value)} placeholder="رقم الهاتف *" className={`${ic} text-right`} />
              ...
              <button disabled={loading} className="gold-btn oneui-btn w-full disabled:opacity-60">{loading ? "..." : "إعادة التعيين"}</button>
              <button type="button" onClick={() => switchView("login")} className="oneui-btn w-full"
          ...
          <form onSubmit={handleLogin}>
            ...
            <div className="space-y-3">
              ...
              {error && <p className="text-sm font-bold text-rose-500">{error}</p>}
              <button disabled={loading} className="gold-btn oneui-btn w-full disabled:opacity-60">{loading ? "..." : "دخول"}</button>
